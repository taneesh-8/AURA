    ...
    for approval in st.session_state.pending_approvals:
        if approval["status"] == "Pending":
            with st.expander(f"🔍 {approval['company_name']} - ${approval['loan_amount']}M", expanded=False):
                ...
                # FIX: Get user role with proper fallback and normalization
                ...
                # Approval actions (ONLY for managers/admins - FIXED)
                if user_role_normalized in ["manager", "admin", "administrator"]:
                    ...
                    st.warning(f"🔒 Only managers and admins can approve/reject applications. Your role: **{user_role}**")
                    st.info("💡 Please contact a manager or admin to review this application.")
else:
